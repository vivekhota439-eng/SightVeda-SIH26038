# SightVeda — Diabetic Retinopathy AI Screening Prototype (SIH26038)
### AI-Assisted Research & Screening Prototype

SightVeda is an end-to-end, AI-assisted diabetic retinopathy screening prototype integrating **image quality screening**, **homomorphic illumination normalization**, **bilateral denoising**, **sub-pixel microaneurysm localization (±0.15 px)**, **5-class deep CNN severity grading**, **weighted cross-entropy class imbalance resolution**, **Grad-CAM visual explainability**, **hybrid clinical rule engine (Referable DR Level 2+)**, **multilingual patient-centric reporting**, **district-scale telemedicine simulation (100,000+ patients/year)**, and an **interactive clinical GUI portal**.

---

> **Prototype status:** This repository is intended for SIH demonstration, research, and prototype development. It is not a medical device or regulatory-certified system and must not be used as the sole basis for diagnosis or treatment. External clinical validation is required before real-world clinical deployment.

## Validation Status

The supplied validation artifact represents 4,293 samples. Current measured results are: **74.38% artifact accuracy**, **0.8479 QWK**, **81.46% referable-DR sensitivity**, and **92.09% specificity**. The team has also observed **78% best prototype accuracy** in its development work. The SIH target is >90% sensitivity and >85% specificity for referable DR; the supplied artifact does not yet meet the sensitivity target.
...
======================================================================
  ...
  [5] Run District Telemedicine Simulation (100,000+ Patients/Year Scenario)
...
IMAGE QUALITY ASSESSMENT (Focus, Brightness, Uniformity, FOV, Vessels)
    ...
    OPTIC DISC & FOVEA LOCALIZATION (CSME Macular Risk Check)
          ↓
    VISUAL EXPLAINABILITY & GRAD-CAM HEATMAP (prototype rapid-review workflow)
          ↓
...
│   ├── generate_explainability.m        # Visual evidence overlays (prototype rapid-review metadata)
...
│
├── simulation/
│   ├── district_simulation.m            # 100,000+ patient/year scenario-based district telemedicine queueing simulation
